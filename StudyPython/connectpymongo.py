...
def merge_contrast(df, df1, last_max_name):
    # df = pd.read_csv("C:/Users/qttu01/Documents/contrast/contrast_20190917183728.csv", encoding="utf_8_sig")
    # df1 = pd.read_csv("C:/Users/qttu01/Documents/contrast/contrast_20190917184557.csv", encoding="utf_8_sig")
    df_1 = df1[df1['state'] == '已导出']
    new_df = pd.concat([df, df_1]).drop_duplicates(['dealercode'], keep='last')
    new_df = pd.DataFrame(new_df)
    new_df.to_csv(last_max_name, encoding="utf_8_sig", index=False)
...
def get_maxDate():
    ...
    for file in contrast_file:
        filename = os.path.basename(file)
        basename_list.append(filename[9:23])
    max_name = max(basename_list)
    return max_name
...
def export_mongodb():
    ...
    max_name = get_maxDate()
    # 先拿到日期最大的表再创建对照表
    file_name = contrast_path + "/contrast_" + max_name + ".csv"
    df = pd.read_csv(file_name, encoding='utf_8_sig')
    time.sleep(1)
    # 创建新的对照表
    create_contrast(fright_df)
    # 拿到最新创建的对照表
    max_name1 = get_maxDate()
    file_name1 = contrast_path + "/contrast_" + max_name1 + ".csv"
    df1 = pd.read_csv(file_name1, encoding='utf_8_sig')
    ...
    if customer_id_list:
        for i in range(len(customer_id_list)):
            ...
            for c in customer:
                df1.loc[df1['email'] == c['email'], 'customersid'] = c['_id']
                df1.to_csv(file_name1, encoding="utf_8_sig", index=False)
    # subsidiaryMark为空的拿出来 这些可以不用去关联销售表匹配subsidiaryMark了
    # 但是需要查看email对应的经销商编码是否会有多个 在对照表中对应的经销商id相通即可
    # 将对照表中subsidiaryMark为空的拿出来
    df_sub_null = df1[df1['subsidiaryMark'].isna()]
    ...
    # 当sub为空时直接导入数据
    find_xiao(email_cusotmer_list, gk_xiao, deaclercode_list, df1, file_name1)
    # find_xiao(email_cusotmer_list, beilin_xiao, deaclercode_list, df)

    df_sub_notnull = df1[df1['subsidiaryMark'].notna()]  # 拿到sub不为空的df
    ...
    # 当sub不为空时直接导入数据
    find_sub_xiao(email_cusotmer_list_1, gk_xiao, sub_notnull_list, dcodesubnonull_list, df1, file_name1)
    # find_sub_xiao(email_cusotmer_list_1, gk_xiao, sub_notnull_list, dcodesubnonull_list, df)

    # 将对照表第二次上传的数据与第一次上传的数据合并
    merge_contrast(df, df1, file_name1)
    print("已完成运行")
...
def find_xiao(email_cusotmer_list, xiao_table, deaclercode_list, df, file_name1):
    for i in range(len(email_cusotmer_list)):
        ...
        if df1.empty:
            print(deaclercode_list[i], "该经销商暂无销售数据")
        else:
...
def find_sub_xiao(email_cusotmer_list_1, xiao_table, sub_notnull_list, dcodesubnonull_list, df, file_name1):
    ...
    for i in range(len(email_cusotmer_list_1)):
        ...
        df1 = pd.DataFrame(list(datas))
        if df1.empty:
            print(dcodesubnonull_list[i], "该经销商暂无销售数据")
        else:
...
def create_contrast(fright_df):
    ...
    contrast_df.to_csv(contrast_path + "/contrast_" + today_name + ".csv",
...
# contrast命名方式
contrast_name_path = contrast_path + "/contrast_" + today_name + ".csv"
# 获取上月第一天的数据
